...
import jmetal.metaheuristics.MOEADD.util.MOEADUtils;

import jmetal.core.Solution;
import jmetal.metaheuristics.MOEADD.util.point.impl.IdealPoint;
import jmetal.metaheuristics.MOEADD.util.point.impl.NadirPoint;
...
public class MOEADD  extends Algorithm{
  ...
  protected SolutionSet jointPopulation;
  ...
  public MOEADD(Problem problem)
{
    ...
    neighborSize=20;
    functionType=MOEADD.FunctionType.PBI;
    dataDirectory=".\\resources\\weightVectorFiles\\moead\\";
  ...
  public void run() throws JMException, ClassNotFoundException{
    ...
    idealPoint = new IdealPoint(problem_.getNumberOfObjectives()); // ideal point for Pareto-based population
    nadirPoint = new NadirPoint(problem_.getNumberOfObjectives()); // nadir point for Pareto-based population
    ...
    for (int curRank = 0; curRank < ranking.getNumberOfSubfronts(); curRank++) {
      SolutionSet front = ranking.getSubfront(curRank);
      for(int i=0;i<front.size();i++) {
    	  Solution s = front.get(i);
    	  //int position = this.population.indexOf(s);
    	  int position = this.population.getSolutionList().indexOf(s);
    ...
    do {
      ...
      for (int i = 0; i < populationSize; i++) {
        ...
        List<Solution> parents = matingSelection(cid, type);

        //parents
        Solution [] parentsArray = new Solution[2];
        parentsArray[0] = parents.get(0);
        parentsArray[1] = parents.get(1);
        
        Solution [] children = (Solution []) crossoverOperator.execute(parentsArray);

        Solution child = children[0];
   ...
   */
  public List<Solution> matingSelection(int cid, int type) {
    int rnd1, rnd2;
 ...
 * @throws JMException 
   */
  public void updateArchive(Solution indiv) throws JMException {

    // find the location of 'indiv'
    setLocation(indiv, idealPoint.getValues(), nadirPoint.getValues());
    int location = (int) indiv.getAdditionalAttributes().get("region");
    ...
    if (numRanks == 1) {
      ...
      ArrayList<Solution> lastFront = new ArrayList<>(populationSize);
      ...
      if (frontSize == 0) {  // the last non-domination level only contains 'indiv'
        ...
        for (int i = 0; i < populationSize; i++) {
          if (rankIdx[numRanks - 1][i] == 1) {
            lastFront.add((Solution) population.get(i));
          }
        }
      ...
      if (frontSize == 1 && lastFront.get(0).equals(indiv)) {  // the last non-domination level only has 'indiv'
        ...
        if (curNC == 1) {  // the subregion only has the solution 'targetIdx', keep solution 'targetIdx'
          ...
          int targetRank = population.get(targetIdx).getRank(); // ranking.getRank(population.get(targetIdx));
          rankIdx[targetRank][targetIdx] = 0;
          rankIdx[indivRank][targetIdx] = 1;

          Solution targetSol = population.get(targetIdx);
          ...
          default:
            ...
            if (list.isEmpty()) {
              ...
              if (idxArray[targetIdx] == -1) {
                ...
                //indiv.getRank();
                int indivRank = indiv.getRank(); //ranking.getRank(indiv);

                //int targetRank = ((DoubleSolution) population.get(idxArray[targetIdx])).getRank();
                int targetRank = population.get(idxArray[targetIdx]).getRank();//ranking.getRank(population.get(idxArray[targetIdx])) ;

                rankIdx[targetRank][idxArray[targetIdx]] = 0;
                rankIdx[indivRank][idxArray[targetIdx]] = 1;

                Solution targetSol = population.get(idxArray[targetIdx]);
        ...
        if (rankIdx[investigateRank][i] == 1) {
          ...
          if (checkDominance(indiv, population.get(i)) == 1) {
            ...
            if (flag == 0) {  // the ith solution can move to the prior level
              ...
              population.get(i).setRank(investigateRank - 1);
            }
          }
        }
      }
            ...
            if (rankIdx[investigateRank][j] == 1) {
              ...
              if (checkDominance(population.get(curIdx), population.get(j)) == 1) {
                ...
                if (flag == 0) {
                  ...
                  population.get(j).setRank(investigateRank - 1);
                }
              }
            }
          }
    ...
    for (int i = 0; i < num_ranks; i++) {
      ...
      if (flag == 1) {  // 'indiv' is non-dominated with all solutions in the ith non-domination level, then 'indiv' belongs to the ith level
        ...
        for (int j = 0; j < curListSize; j++) {
          ...
          population.get(curIdx).setRank(newRank);
        }
        for (int j = 0; j < populationSize; j++) {
          ...
          if (allFlag == 1) {  // move the solutions after the 'prevRank'th level to their next levels
            ...
            while (curRank < num_ranks) {
              ...
              for (int j = 0; j < level_size; j++) {
                ...
                tempRank =  population.get(curIdx).getRank();
                newRank = tempRank + 1;
                //((DoubleSolution) population.get(curIdx)).setRank(newRank);
                //population.get(curIdx).attributes().put(ranking.getAttributedId(), newRank);
                population.get(curIdx).setRank(newRank);
        ...
        while (i < num_ranks) {
          ...
          for (int j = 0; j < level_size; j++) {
            ...
            curRank = population.get(curIdx).getRank();
            newRank = curRank + 1;
            //((DoubleSolution) population.get(curIdx)).setRank(newRank);
            //population.get(curIdx).attributes().put(ranking.getAttributedId(), newRank);
            population.get(curIdx).setRank(newRank);
    ...
    switch (flag) {
      ...
      case 2:
        ...
        for (int i = 0; i < tempSize; i++) {
          ...
          population.get(curIdx).setRank(level + 1);
        ...
        //indiv.attributes().put(ranking.getAttributedId(), level);
          ...
          if (rankIdx[level][i] == 1) {
            ...
            //population.get(i).attributes().put(ranking.getAttributedId(), level + 1);
        	  population.get(i).setRank(level + 1);
        	  rankIdx[level][i] = 0;
            rankIdx[level + 1][i] = 1;
          }
        }
 ...
 * @throws JMException 
   */
  public void deleteCrowdRegion1(Solution indiv, int location) throws JMException {
    ...
    int maxRank = population.get(indList.get(0)).getRank();
    maxRankList.add(indList.get(0));
    for (int i = 1; i < indList.size(); i++) {
      int curRank =  population.get(indList.get(i)).getRank();
    ...
    int targetRank = population.get(targetIdx).getRank();
 ...
 * @throws JMException 
   */
  public void deleteCrowdRegion2(Solution indiv, int location) throws JMException {
    ...
    for (int i = 1; i < indList.size(); i++) {
      ...
      if (indList.get(i) == -1) {
        ...
        curRank =  population.get(indList.get(i)).getRank();
 ...
 * @throws JMException 
   */
  public void deleteRankOne(Solution indiv, int location) throws JMException {
 ...
 * @throws JMException 
   */
  public double sumFitness(int location) throws JMException {
 ...
 * @throws JMException 
   */
  public void deleteCrowdIndiv_same(int crowdIdx, int nicheCount, double indivFitness, Solution indiv) throws JMException {
 ...
 * @throws JMException 
   */
  public void deleteCrowdIndiv_diff(int crowdIdx, int curLocation, int nicheCount, Solution indiv) throws JMException {
  ...
  public void setLocation(Solution indiv, double[] z_, double[] nz_) {
    ...
    //indiv.setAdditionalAttributes("region", minIdx);
    indiv.getAdditionalAttributes().put("region", minIdx);
...
//        indiv.setAttribute(ATTRIBUTES.DIST, minDist);
  ...
  public void replace(int position, Solution solution) {
    if (position > this.population.size()) {
      ...
      /*Solution toRemove = population.get(position);
      population.remove(toRemove);
      population.add(position, solution);*/
    	population.replace(position, solution);
 ...
 * @throws JMException 
   */
  protected void initializeUniformWeight() throws JMException {
    if ((problem_.getNumberOfObjectives() == 2) && (populationSize <= 300)) {
      ...
      try {
        ...
        throw new JMException("initializeUniformWeight: failed when reading for file: "
  ...
  double fitnessFunction(Solution individual, double[] lambda) throws JMException {
	    ...
	    if (MOEADD.FunctionType.TCHE.equals(functionType)) {
	      ...
	      fitness = maxFun;
	    } else if (MOEADD.FunctionType.AGG.equals(functionType)) {
	      double sum = 0.0;
	      for (int n = 0; n < problem_.getNumberOfObjectives(); n++) {
	    ...
	    } else if (MOEADD.FunctionType.PBI.equals(functionType)) {
	      double d1, d2, nl;
	      double theta = 5.0;
	      ...
	      throw new JMException(" MOEAD.fitnessFunction: unknown type " + functionType);
	    }
	    return fitness;
	  }
